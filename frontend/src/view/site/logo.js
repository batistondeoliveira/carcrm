...
export default function Logo() {
    ...
    const _handleUpload = (event) => {
        ...
        dispatch(uploadLogo(body)).then(response => response && setState({ isUpload: null }));
    }

    const _destroy = (id) => {
        setState({ isDeleted: id });

        dispatch(destroyLogo(id)).then(response => response && setState({ isDeleted: null }));
    }
            ...
            <div className="card-body scroll">
                <div className="d-flex flex-column align-items-center mt-5">
                    {(app.logo) 
                        ? 
                            <>
                                <img alt="" style={(state.isDeleted) ? {opacity: 0.5} : {}} className="img-fluid" src={rootUrl + 'thumb/logo/' + app.logo + '?user=' + app.id + '&height=150'} />
