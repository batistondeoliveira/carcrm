...
export default function OwnerVehicles(props) {
    ...
    const _index = (loadMore) => {
        dispatch(vehicles(query, loadMore)).then(response => {
            setLoading(false);
            setLoadMore(false);
        })
    }
                        ...
                        <>
                            {(list.data.length > 0) &&
                                <div className="card-body">
                                    <h6 className="m-0">
                                        {list.total} {(list.total > 1) ? 'veículos encontrados' : 'veículo encontrado'}
                                    </h6>
                                </div>
                            }
                                ...
                                <React.Fragment key={index}>
                                    <div className="card-body d-flex">
                                        <div className="vehicle-img d-flex justify-content-center align-items-center">
                                            { (item.cover && <img alt="" className="shadow rounded" src={rootUrl + 'thumb/vehicles/' + item.cover.img + '?user=' + item.cover.user_id + '&subdir=' + item.cover.vehicle_id + '&width=180&height=135'} />) }
                                        </div>
